...
本技能可能在两种上下文中执行。先判断模式：若当前可用工具清单不含 `AskUserQuestion`（典型即由 `neo` agent 的 `skills:` frontmatter 加载本技能），则为自主模式；否则为交互模式。
...
5. 使用一个最终选项问题确认共同理解并请求实施授权（自主模式下：停下回报该候选决策，由主线程转问用户，确认后再继续）。
...
- 自主模式：每次回报一个候选决策，附推荐答案与仓库证据。正文所有"用选项问题让用户决定"的表述在自主模式下均映射为：停下回报一个候选决策，由主线程转问用户。
