...
在 Claude Code 交互会话中输入以下命令：

```
/config env.QCC_API_KEY "your-api-key-here"
```
...
在 Claude Code 交互会话中逐条输入：

```
/config env.ANTHROPIC_AUTH_TOKEN "sk-your-deepseek-api-key"
/config env.ANTHROPIC_BASE_URL "https://api.deepseek.com/anthropic"
/config env.ANTHROPIC_MODEL "deepseek-v4-pro[1m]"
/config env.ANTHROPIC_DEFAULT_OPUS_MODEL "deepseek-v4-pro[1m]"
/config env.ANTHROPIC_DEFAULT_SONNET_MODEL "deepseek-v4-pro[1m]"
/config env.ANTHROPIC_DEFAULT_HAIKU_MODEL "deepseek-v4-flash"
```
...
```
/config env.DASHSCOPE_API_KEY "sk-your-dashscope-key"
```
...
```
/config env.TAVILY_API_KEY "tvly-your-tavily-key"
```
...
```
/config env.CLAUDE_CODE_SUBAGENT_MODEL "deepseek-v4-flash"
...
| API Key 不生效          | 配置在了系统环境变量而非 settings.json             | 确认使用 `/config env.XXX` 斜杠命令或手动编辑 `~/.claude/settings.json` 的 `env` 字段 |
...
| settings.json 配置丢失  | 手动编辑时 JSON 格式错误                           | 使用 `/config` 斜杠命令代替手动编辑，或用 `python -m json.tool` 验证 JSON 格式    |
