...
3. **配置**：运行 `python scripts/set_baidu_key.py <用户提供的 key>`（脚本位于本技能 `scripts/` 目录，先定位再执行）。脚本会自动检测运行环境：在 Claude Code 中把 key 写入 `~/.claude/settings.json` 的 `env` 字段；在其他环境（Claude Desktop、Cursor、其他 MCP 客户端或普通终端）则设置用户级环境变量——Windows 通过 PowerShell 的 `[System.Environment]::SetEnvironmentVariable("BAIDU_API_KEY", <key>, "User")` 写入注册表，Linux/macOS 按登录 shell 把 `export BAIDU_API_KEY="<key>"` 追加到 `~/.zshrc`（zsh）或 `~/.bashrc`（其他 shell）。仅依据脚本返回值判断是否设置成功：返回码 0 表示成功，继续下一步；非 0 则报告脚本错误信息，提示修复后重试。
4. **验证**：按脚本输出的「生效方式」提示操作——Claude Code 环境：重启 Claude Code（或 `/mcp` 重新连接），确认工具列表出现 `mcp__plugin_baidu-search_*` 工具；非 Claude Code 环境：Windows 新开终端，Linux/macOS 执行 `source ~/.zshrc` / `source ~/.bashrc`，然后重启相应的 MCP 客户端并确认其工具可用。仍不可用则提示排查网络、key 有效性。
