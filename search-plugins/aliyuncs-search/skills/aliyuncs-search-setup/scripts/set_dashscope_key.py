#!/usr/bin/env python3
"""
Set DASHSCOPE_API_KEY for the current environment.
...
* Anywhere else -> sets a persistent user-level environment variable:
  - Windows: [System.Environment]::SetEnvironmentVariable("DASHSCOPE_API_KEY",
    value, "User") via PowerShell (registry-backed, survives reboots);
  - Linux/macOS: appends `export DASHSCOPE_API_KEY="value"` to the rc file
...
def main():
    if len(sys.argv) != 2:
        print(f"Usage: python set_dashscope_key.py <{VAR_NAME} value>", file=sys.stderr)
        sys.exit(1)
