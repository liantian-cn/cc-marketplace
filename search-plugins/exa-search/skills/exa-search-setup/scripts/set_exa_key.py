#!/usr/bin/env python3
"""
Set EXA_API_KEY for the current environment.

Usage:
  python set_exa_key.py <EXA_API_KEY value>
...
* Anywhere else -> sets a persistent user-level environment variable:
  - Windows: [System.Environment]::SetEnvironmentVariable("EXA_API_KEY",
    value, "User") via PowerShell (registry-backed, survives reboots);
  - Linux/macOS: appends `export EXA_API_KEY="value"` to the rc file of
...
def main():
    if len(sys.argv) != 2:
        print(f"Usage: python set_exa_key.py <{VAR_NAME} value>", file=sys.stderr)
        sys.exit(1)
