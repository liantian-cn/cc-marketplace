#!/usr/bin/env python3
"""
Set FIRECRAWL_API_KEY for the current environment.

Usage:
  python set_firecrawl_key.py <FIRECRAWL_API_KEY value>
...
* Anywhere else -> sets a persistent user-level environment variable:
  - Windows: [System.Environment]::SetEnvironmentVariable("FIRECRAWL_API_KEY",
    value, "User") via PowerShell (registry-backed, survives reboots);
  - Linux/macOS: appends `export FIRECRAWL_API_KEY="value"` to the rc file
    of the user's login shell (~/.zshrc for zsh, ~/.bashrc otherwise),
...
def main():
    if len(sys.argv) != 2:
        print(f"Usage: python set_firecrawl_key.py <{VAR_NAME} value>", file=sys.stderr)
        sys.exit(1)
