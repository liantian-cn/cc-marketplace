#!/usr/bin/env python3
"""
Set PKU_LAW_API for the current environment.
...
* Anywhere else -> sets a persistent user-level environment variable:
  - Windows: [System.Environment]::SetEnvironmentVariable("PKU_LAW_API",
    value, "User") via PowerShell (registry-backed, survives reboots);
  - Linux/macOS: appends `export PKU_LAW_API="value"` to the rc file of
...
def main():
    if len(sys.argv) != 2:
        print(f"Usage: python set_pkulaw_key.py <{VAR_NAME} value>", file=sys.stderr)
        sys.exit(1)
